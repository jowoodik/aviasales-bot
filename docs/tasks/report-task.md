# Документация NotificationService - Обновлённая версия

## Обзор системы

NotificationService реализует приоритетную систему уведомлений с:
- **4 уровня приоритетов**: CRITICAL, HIGH, MEDIUM, LOW
- **Адаптивное форматирование** под каждый приоритет
- **Inline-кнопки** для быстрого перехода к покупке
- **Дайджесты** для отложенных уведомлений
- **Ночной режим** с беззвучными CRITICAL-уведомлениями
- **Лимиты** для Free подписки (3 CRITICAL в день)
- **Cooldown** для HIGH уведомлений (раз в 3 часа для Plus)

---

## Структура базы данных

### Таблица `notification_log`
```sql
CREATE TABLE IF NOT EXISTS notification_log (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  chat_id INTEGER NOT NULL,
  route_id INTEGER,
  priority TEXT NOT NULL,        -- 'CRITICAL', 'HIGH', 'MEDIUM', 'LOW'
  price REAL,
  message_type TEXT NOT NULL,    -- 'instant', 'digest', 'report'
  sent_at DATETIME DEFAULT (datetime('now')),
  disable_notification INTEGER DEFAULT 0
);
```

### Таблица `daily_digest_queue`
```sql
CREATE TABLE IF NOT EXISTS daily_digest_queue (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  chat_id INTEGER NOT NULL,
  route_id INTEGER NOT NULL,
  priority TEXT NOT NULL,
  price REAL NOT NULL,
  avg_price REAL,
  historical_min REAL,
  best_result_id INTEGER,
  created_at DATETIME DEFAULT (datetime('now')),
  processed INTEGER DEFAULT 0
);
```

---

## Классификация приоритетов

### Метод: `classifyPriority(routeData)`

**Возвращает:**
```javascript
{
  priority: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW',
  reasons: string[]
}
```

### Логика приоритетов

#### CRITICAL
- Цена <= бюджет пользователя
- Цена = исторический минимум
- Скидка >= 50% от средней цены

#### HIGH
- Превышение бюджета <= 15%
- Скидка 30-49% от средней цены
- Падение >= 15% за 24ч

#### MEDIUM
- Превышение бюджета 15-30%
- Скидка 15-29% от средней цены
- Падение 10-14% за 24ч

#### LOW
- Всё остальное

---

## Маршрутизация уведомлений

### Метод: `processAndRouteNotification(...)`

**Логика для каждого приоритета:**

| Приоритет | Ночью | Free подписка | Plus подписка |
|-----------|-------|---------------|---------------|
| **CRITICAL** | Беззвучно | Макс 3/день, затем → дайджест | Без лимита, со звуком |
| **HIGH** | → Дайджест | → Дайджест | Раз в 3ч (беззвучно), затем → дайджест |
| **MEDIUM** | → Дайджест | → Дайджест | → Дайджест |
| **LOW** | Тихо | Тихо | Тихо |

**Ночное время:** 23:00 - 08:00

---

## Форматирование уведомлений

### CRITICAL: Максимально продающий формат

```
🔥🔥🔥 СУПЕР ЦЕНА! 🔥🔥🔥

<b>Город → Город</b>
📅 дд.мм–дд.мм (N дней)
👥 X взрослых [+ Y детей]

💎 <b>XXX XXX ₽</b> за всех

<b>💰 Экономия XX XXX ₽ (-XX%)</b>

🎯 Ваш бюджет: XXX XXX ₽ [✅ или (+X%)]
📊 Обычная цена: XXX XXX ₽

✈️ Авиакомпания -  Тип рейса -  🧳

⚡️ <b>Цена может вырасти в ближайшие часы</b>

[🎫 КУПИТЬ СЕЙЧАС] ← inline-кнопка
```

**Особенности:**
- Яркий заголовок с тремя огнями
- Крупная цена сразу после маршрута
- Акцент на экономии жирным шрифтом
- Элемент срочности внизу
- Inline-кнопка "КУПИТЬ СЕЙЧАС"

### HIGH: Нейтральный информативный формат

```
📊 Хорошая цена найдена -  ЧЧ:ММ

<b>Город → Город</b>
📅 дд.мм–дд.мм (N дней)
👥 X+Y -  Пересадки -  🧳

💰 <b>XXX XXX ₽</b> за всех

Ваш бюджет: XXX XXX ₽ [(+X%) или ✅]
Средняя цена: XXX XXX ₽ [(-X%)]

✈️ Авиакомпания

💡 Продолжаю искать варианты в бюджете

[🎫 Посмотреть билет] ← inline-кнопка
```

**Особенности:**
- Нейтральный заголовок со временем
- Компактная информация о пассажирах
- Два сравнения (бюджет и средняя)
- Успокаивающее "Продолжаю искать"
- Обычная кнопка без CAPS LOCK

### MEDIUM/LOW: Минимальный формат

```
🔍 Проверка завершена -  ЧЧ:ММ

<b>Город → Город</b>
📅 дд.мм–дд.мм
👥 X+Y

Цена: XXX XXX ₽
Ваш бюджет: XXX XXX ₽ [(превышение в X раза) или (+X%)]

Продолжаю мониторинг 🔎
```

**Особенности:**
- Минимум информации
- Только цена и бюджет
- Нет призывов к покупке
- Фокус на продолжении поиска

### Нет цен

```
📊 Проверка завершена -  ЧЧ:ММ

<b>Город → Город</b>
❌ Цены не найдены
Ваш бюджет: XXX XXX ₽

Сейчас выполнено X проверок. Всего проверок Y

Продолжаю мониторинг 🔍
```

---

## Inline-кнопки

### Одиночное уведомление

Кнопка прикрепляется к сообщению через `reply_markup`:

```javascript
const sendOpts = {
  parse_mode: 'HTML',
  disable_notification: silent,
  reply_markup: {
    inline_keyboard: [[
      { 
        text: priority === 'CRITICAL' ? '🎫 КУПИТЬ СЕЙЧАС' : '🎫 Посмотреть билет',
        url: bestResult.search_link 
      }
    ]]
  }
};
```

### Сводный отчёт

Кнопки собираются для всех маршрутов (максимум 10):

```javascript
const buttons = [];
for (const rb of routeBlocks) {
  if (rb.block.searchLink && buttons.length < 10) {
    const routeName = airportResolver.formatRoute(rb.route.origin, rb.route.destination);
    buttons.push([{
      text: `🔗 ${routeName} — Смотреть →`,
      url: rb.block.searchLink
    }]);
  }
}

opts.reply_markup = { inline_keyboard: buttons };
```

Кнопки прикрепляются только к последнему чанку.

---

## Сводный отчёт

### Разделители с самолётом

Вместо статичных линий `━━━━━━` используется динамический самолёт:

```javascript
const generatePlaneLine = (position, totalSteps) => {
  const LINE_LENGTH = 20;
  const planePos = Math.round((position / totalSteps) * LINE_LENGTH);
  const left = '─'.repeat(planePos);
  const right = '─'.repeat(LINE_LENGTH - planePos);
  return `<b>${left}✈${right}</b>`;
};
```

**Примеры:**
- 0 из 3: `<b>✈────────────────────</b>`
- 1 из 3: `<b>──────✈─────────────</b>`
- 2 из 3: `<b>─────────────✈──────</b>`
- 3 из 3: `<b>────────────────────✈</b>`

### Заголовок

```
🔥 Отличные новости! -  ЧЧ:ММ    (если есть CRITICAL)
📊 Проверка завершена -  ЧЧ:ММ   (обычная)

<b>✈────────────────────</b>
```

### Подвал

```
<b>────────────────────✈</b>

Отличные цены! Не упусти 🎯     (если есть находки)
Продолжаю мониторинг 🔍         (если находок нет)
```

---

## Дайджест

### Метод: `sendDigestForUser(chatId)`

**Когда отправляется:**
- Free: 1 раз в день в 10:00
- Plus: 2 раза в день в 10:00 и 18:00

**Логика:**
1. Получить непроцессированные элементы из `daily_digest_queue`
2. Отсортировать по приоритету (CRITICAL → HIGH → MEDIUM → LOW)
3. Для каждого сформировать блок
4. Отправить через `sendConsolidatedReport()`
5. Пометить `processed = 1`

**Звук:** Всегда беззвучно

---

## Форматирование даты и времени

### Короткая дата
```javascript
_formatShortDateForProgressBar(dateStr) // → "1.05", "25.12"
```

### Длинная дата
```javascript
_formatShortDateRu(dateStr) // → "1 мая", "25 декабря"
```

### Время
```javascript
_formatTimeForUser(date, timezone) // → "16:52"
```

### Множественное число дней
```javascript
_pluralizeDays(7)  // → "дней"
_pluralizeDays(1)  // → "день"
_pluralizeDays(22) // → "дня"
```

---

## Разбиение длинных сообщений

### Метод: `_splitMessage(text, maxLength)`

Telegram лимит: 4096 символов

**Логика:**
1. Если длина <= 4000 → возвращаем как есть
2. Разбиваем по разделителям
3. Группируем в чанки
4. Inline-кнопки только к последнему чанку

---

## Настройки пользователя

### Таблица `user_settings`

```sql
night_mode INTEGER DEFAULT 1
notifications_enabled INTEGER DEFAULT 1
digest_enabled INTEGER DEFAULT 1
timezone TEXT DEFAULT 'Asia/Yekaterinburg'
```

### Ночной режим

```javascript
_isNightTime(timezone, settings) // 23:00 - 08:00
```

---

## Полный workflow

```
1. scheduler.js: checkUserRoutes(chatId)
   ↓
2. UnifiedMonitor.checkSingleRoute(route)
   → Сохранение результатов
   ↓
3. NotificationService.getRouteAnalytics(routeId)
   NotificationService.getPriceDropPercent(routeId, currentPrice)
   ↓
4. NotificationService.classifyPriority(routeData)
   → { priority: 'CRITICAL', reasons: [...] }
   ↓
5. NotificationService.processAndRouteNotification(...)
   → Решение: 'sent', 'sent_silent', 'digest', 'skipped', 'silent'
   ↓
6a. Если 'sent'/'sent_silent':
    formatSingleRouteBlock(..., priority)
    → _sendInstantAlert(...) с inline-кнопкой

6b. Если 'digest':
    → _addToDigestQueue(...)

6c. Если 'skipped'/'silent':
    → Ничего
```

---

## Ключевые отличия от предыдущей версии

### Убрано
- ❌ Прогресс-бары `[███████░░]`
- ❌ Индикаторы 🟢/🔴 перед каждой строкой
- ❌ Дублирование цены в расчётах
- ❌ Текстовые ссылки внутри блока
- ❌ Универсальный формат для всех приоритетов

### Добавлено
- ✅ Разные форматы для CRITICAL/HIGH/MEDIUM/LOW
- ✅ Inline-кнопки вместо текстовых ссылок
- ✅ Элементы срочности для CRITICAL
- ✅ Акцент на экономии в рублях
- ✅ Динамический самолёт в разделителях
- ✅ Количество дней для длительных поездок
- ✅ Адаптивные заголовки и подвалы

### Психология
- **CRITICAL**: Максимальная мотивация к покупке, срочность
- **HIGH**: Нейтральное информирование, без паники
- **MEDIUM/LOW**: Минимум информации, фокус на продолжении поиска

---

## Частота отправки

### Сводный отчёт (sendConsolidatedReport)

**Когда:** При каждой плановой проверке маршрутов

**Частота по подпискам:**
- Free: каждые 4 часа
- Plus: каждые 2 часа
- Admin: каждый час

**Условия:**
- Уведомления включены (notifications_enabled = 1)
- Не ночное время (8:00 - 23:00)
- Есть маршруты, которые не были отправлены индивидуально

**Звук:** Беззвучно (disable_notification = true)

### Дайджест (sendDigestForUser)

**Когда:** По фиксированному расписанию в локальное время пользователя

**Частота по подпискам:**
- Free: 10:00
- Plus: 10:00, 18:00

**Содержимое:** Накопленные уведомления из daily_digest_queue

**Звук:** Беззвучно
